...
if (isset($_SESSION['intentos_login'])) {
    $intentos_fallidos = $_SESSION['intentos_login'];
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !$bloqueado) {
    ...
    if (empty($usuario) || empty($clave)) {
        ...
        $stmt = $conn->prepare("SELECT IdUsuario, Nombre, Apellido, Correo, Usuario, Clave, Rol, Estado FROM tb_usuarios WHERE Usuario = ?");
        $stmt->bind_param("s", $usuario);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($id, $nombre, $apellido, $correo, $usuario_db, $hashed_password, $rol, $estado);
            $stmt->fetch();

            if ($estado === 'Activo') {
                $password_valid = false;
                if (password_get_info($hashed_password)['algo']) {
                    $password_valid = password_verify($clave, $hashed_password);
                } else {
                    $password_valid = ($clave === $hashed_password);
                    if ($password_valid) {
                        $new_hash = password_hash($clave, PASSWORD_DEFAULT);
                        $update_stmt = $conn->prepare("UPDATE tb_usuarios SET Clave = ? WHERE IdUsuario = ?");
                ...
                if ($password_valid) {
                    ...
                    $_SESSION['intentos_login'] = $intentos_fallidos + 1;
                }
            } else {
        ...
        if (isset($_SESSION['intentos_login']) && $_SESSION['intentos_login'] >= 5) {
